...
demo_date: "TBD"        # AE doc: "Demo is being scheduled. Hopefully later the week of Aug 24th" — no meeting booked on calendar as of brief generation
demo_time: ""
attendees:
...
generated: "2026-08-25T13:47:36Z"
---
...
Stellantis Financial Services just split off Enterprise Data Management (Nick, 3 months in
role) is racing to migrate ~700 homegrown SSIS packages onto Microsoft Fabric while the team
triples in size, and their single biggest complaint about the system they built themselves is
that failure recovery is manual and replay is weak — with SFS now underwriting up to eight auto
ABS securitization deals in 2026, an audit-ready, loan-level data tape is not optional. This demo
has to prove that Dagster's asset graph gives them one thing their homegrown orchestrator
structurally can't: a partition-scoped, idempotent recovery story — rematerialize the one late
vendor file, not the whole 700-job chain — plus the lineage and blocking checks that keep bad
loan data from ever reaching a securitization pool calculation. If Nick and Chris leave believing
"our failure-recovery problem goes away without a rewrite," the demo has done its job.

## Meeting

- **When:** Not booked yet — AE (Austin) doc says "hopefully later the week of Aug 24th." No
  calendar event exists as of this brief. Intro call already happened Aug 21, 2026 (Gong-recorded).
...
Stated ask for the SE: model their vendor-file → bronze → silver → gold flow and show how an
operator recovers from a failed step without rerunning everything.
...
| Transformation | SQL Server stored procedures / SSIS (~700 packages), bronze/silver/gold medallion pattern | AE notes |
| Ingestion | Vendor file drops (dealer, servicer, credit-bureau feeds implied) via SSIS | AE notes |
| BI | Not stated by AE; Power BI confirmed elsewhere in Stellantis job postings | inferred, low |
...
prime/near-prime auto ABS and lease ABS securitization deals this year. Scale/cadence not stated
by AE; ~700 SSIS packages and a team grown from ~7 to ~35 developers implies daily-batch,
many-source ingestion at meaningful volume. Treat volumes as unknown/low-confidence and use
industry-typical auto-finance cardinalities (thousands of contracts/day, dozens of dealer/vendor
feeds).

## Public signals

- **Job postings:** At least 2 distinct open data roles found — a Data Engineer on the "Customer
  Data Platform" team (Auburn Hills; Azure/Snowflake/BigQuery/AWS, Python/Spark, open through
  9/22/2026) and a Senior Finance Data Engineer/Data Analyst supporting FP&A (Power BI-based
  reporting). Two open reqs across two different data teams is a real, if modest, signal of active
  investment — not a six-person hiring spree, but not nothing.
...
- **Recent news:** SFS is the automaker's captive finance company, built from the 2021-22
  ...
  in 2026 — a concrete, dated reason accurate loan-level data matters this year, not abstractly.
...
Model the medallion architecture (bronze → silver → gold) the AE notes explicitly ask for, themed
on auto-loan/lease servicing feeding ABS pool reporting. Partition on
`MultiPartitionsDefinition` of `date` (daily) × `vendor_source` (a handful of feeds — e.g.
`dealer_originations`, `servicer_payments`, `credit_bureau`, `dealer_floorplan`) so the recovery
story is genuinely partition-scoped, matching "replay/backfill" as a named pain.

**Bronze (raw ingestion, one asset per vendor feed, partitioned date × source):**
...
**Silver (dbt staging models, conformed/cleaned):**
...
**Reporting:**
- `powerbi_portfolio_dashboard_refresh` — represents the exec-facing BI layer

~16 assets total, within the 12–25 target, spanning ingestion → dbt lineage → BI.

## Native integrations to use

- `dagster-dbt` — real dbt Core project against DuckDB for every silver/gold model. Non-negotiable
  per house rules; this is also literally what "bronze/silver/gold" means to this prospect.
- `dagster-msteams` — SFS explicitly said Teams/email are how they'd want notifications; this is a
  native integration that maps directly to a stated requirement, not a generic add-on. Wire an
  alerting hook off the blocking check and the freshness policies.
- `dagster-azure` (ADLS2 resource) — represents the OneLake/ADLS-compatible storage layer under
  Fabric for demo-mode kind badging on ingestion assets, subclassed per the demo_mode pattern.

## Community components to search for

Search terms (not IDs): "microsoft fabric", "onelake", "azure data lake storage gen2", "power bi
refresh", "sql server", "mssql", "teams webhook notification", "email notification asset check".
Native `dagster-azure`/`dagster-msteams` likely cover most of this — only drop to the community
registry (rung 2/3 of the ladder) if a Fabric-specific or Power BI refresh component exists and is
a better fit than hand-rolling a resource.

## Asset checks

At least these three, each mapped to a named pain:

1. **Blocking** — `raw_loan_originations` required-field completeness (loan_id, amount, dealer_id
   non-null) before `stg_loan_originations` computes. Maps to: *"failure recovery is manual, replay
   is weak"* — show that bad data structurally cannot silently propagate.
...
- **What must be mocked:** Microsoft Fabric / OneLake, the vendor file drops (dealer, servicer,
  credit-bureau feeds), and Teams notifications (log/no-op the webhook call in demo mode).
- **What can be real:** DuckDB as the execution engine, the dbt project and its lineage, all
  asset checks, freshness policies, and automation conditions.
- **Asset kinds to display:** `azure` on bronze ingestion assets (OneLake/ADLS stand-in, max 3
  kinds); `dbt` + `azure` on silver/gold dbt models via the translator override (dbt's own
  `adapter_type` will say DuckDB — must be overridden per `templates/demo_mode_pattern.py`);
  `powerbi` on the reporting asset. Do not badge anything `duckdb`.
- **Failure demonstration: yes.** This is the one explicit, named ask from the AE's meeting-agenda
  notes: *"how operators recover from a failed step without rerunning everything."* This is not a
  generic capability we're choosing to showcase — it's literally what they asked the SE to
  demonstrate.
- **If yes — planted anomaly:** One `raw_dealer_floorplan_feed` partition (a specific `date` ×
  ...
  on that bronze asset fails and downstream silver/gold for that partition never computes.
  Recovery is: the mock vendor source is updated (simulating the dealer resending a corrected
  file), and Nick/Chris watch you rematerialize only that one partition — not the other
  vendor-source partitions for that date, and not any other date. No heal asset, no reset job —
  the fix lives in `demo_data/`, exactly per house rules.
...
Screen: the full bronze → silver → gold asset graph, green, with `abs_pool_eligibility` and
`fact_delinquency_snapshot` showing freshness policies satisfied, dbt lineage rendered natively,
and Azure/dbt/Power BI kind badges making it visually read as their stack. Then: select the one
`raw_dealer_floorplan_feed` partition with the planted anomaly, show the blocking check red and
downstream silver/gold refusing to compute for that partition only, "fix" the mock vendor file,
and rematerialize just that partition — narrate that this is the entire recovery procedure, no
custom tooling, no reset step, no rerun of the other 699 packages' worth of work.

## Capability talk track

- **Asset checks (blocking):** *"This is your failure-recovery problem solved structurally — bad
  data physically cannot reach the ABS pool calculation, because the check gates it before
...
- **Automation conditions (`AutomationCondition.eager()`):** *"This is what happens automatically
  when a corrected vendor file lands — you don't have to remember to re-run anything."*
- **`dagster-msteams` alerting hook:** *"This posts to the same Teams channel your team already
  lives in — no new tool to check."*
- **dbt lineage:** *"This is column- and model-level lineage across your medallion layers, for
  free, because you already write dbt models."*

## Explicitly out of scope

Power BI is represented as a single refresh-trigger asset, not a real embedded report — building
an actual Power BI workspace integration is out of scope for a one-night build. Multi-tenant
RBAC/SSO and branch deployments (raised in older Kiewit-style enterprise conversations, not this
one) are not part of this brief. Credit-bureau data realism is illustrative only — no real bureau
schemas or scoring logic. Do not build anything resembling GLBA compliance tooling itself; the
demo shows data quality and lineage, not a compliance product.
