...
<!-- REWRITE 2026-08-26 (third pass): Eric deleted both this brief and the
generated project a second time ("Clear stellantis brief and project,
requeue", commit 49cfdf2) immediately after landing "Rung 3 is not optional —
subclass before you write custom" in CLAUDE.md (commit 2b422fb). The all-green
second rebuild (PR #9, dd6ea35) was itself fine on the "demos always work"
rule, but its own component-feedback file
(`component-feedback/2026-08-26-fabric-trigger-and-observe-asset.md`)
rejected the registry's `fabric_workspace` component for exactly the three
reasons CLAUDE.md's Rung 3 table now names as NOT disqualifying — "discovers
items from a live connection," "asset keys come from the source system, not
us," and "no partitions support" — and went straight to a custom
`FabricPipelineAssetComponent` (rung 4), skipping the subclass rung (rung 3)
entirely. LEARNINGS.md was updated in the same sitting to record this
explicitly as the case to not repeat. This rewrite keeps everything else
unchanged (research re-checked again 2026-08-26 — same AE doc, same content,
still no calendar event; the demo thesis; the asset graph; the all-green demo
mode) and replaces the component-strategy build directives with an explicit
subclass-first mandate, so tonight's build doesn't reach the same conclusion
the last one did. -->
...
- **When:** Not booked yet — AE (Austin) doc says "hopefully later the week of Aug 24th." No
  calendar event exists as of this rewrite (checked a third time, 2026-08-26 — same as both prior
  passes). Intro call already happened Aug 21, 2026 (Gong-recorded).
...
- **No meeting booked yet** — re-checked calendar and Drive again for this rewrite (2026-08-26),
  still nothing scheduled and no new AE material beyond the same Aug 24 doc (unchanged since the
  last two passes). The AE notes mention a "wish list" the prospect offered to share; it still
...
**This is the section that caused the last two rebuilds. Get it right this time.**

The 2026-08-26 build (PR #9) rejected the registry's `fabric_workspace` component and wrote a
custom `FabricPipelineAssetComponent` instead. Its own written justification
(`component-feedback/2026-08-26-fabric-trigger-and-observe-asset.md`) gave three reasons — "it
discovers items from a live connection," "asset keys come from the source system, not us," "no
partitions support." **All three are now named explicitly in `CLAUDE.md`'s Rung 3 table as
objections that are NOT disqualifying.** Each is a subclass away:

- *Live discovery* → subclass `fabric_workspace`, override the discovery/list call, return a fixed
  item list in demo mode (this is the same demo_mode seam every other component in this repo
  uses — see `templates/demo_mode_pattern.py`).
- *Asset keys from the source system* → add an explicit mapping in the subclass — an
  `assets_by_item_name`-style config field binding each of the ~17 Fabric pipeline items to
  the asset key, deps, partitions, and metadata this brief specifies. This is precisely the
  `assets_by_task_key` pattern in the Databricks workspace component
...
**Do not repeat the rung-4 jump.** Subclass `fabric_workspace` (rung 3) as **one component
instance** covering all ~17 items via the explicit mapping table in `defs.yaml` — not one instance
per pipeline, and not a bespoke `FabricPipelineAssetComponent` reimplementing what the registry
component already does. Compose freshness policies, retry policies, and automation conditions
from outside the component via `defs.yaml`, the same way `AssetSpec`-based components already do
elsewhere in this repo — that's a config/wiring concern, not a reason to reject the base
component.
...
custom component — and write the feedback file per `CLAUDE.md`'s "When you write a custom
component, write feedback" section, including the literal subclass attempt that didn't work, not
just registry-search terms.

The second custom component from the last build, `DuckDbAssetCheckComponent` (an arbitrary
...
registry search from scratch, the prior run's searches already covered this domain adequately;
the gap was in the escalation decision, not the search.
...
- **What must be mocked:** The Fabric pipeline trigger/poll/discovery API itself — in demo mode,
  the subclassed `fabric_workspace` component's discovery call returns the fixed ~17-item list
  from the mapping table instead of hitting a live workspace, and "triggering a Fabric pipeline"
  runs a local synthetic-data function with the same run/poll/complete lifecycle a real
  trigger-and-observe call would have — plus the vendor file drops. This is the network boundary
  to fake, per `templates/demo_mode_pattern.py`; the component class, asset keys, partitions,
  checks, and YAML schema stay identical between demo and real mode.
...
call, not a failure. Also worth 20 seconds: open the `defs.yaml` mapping table itself and show
that adding SFS's 30th (or 700th) pipeline is one more entry in that file, not a new Python class —
directly answers "is this bespoke per pipeline."
