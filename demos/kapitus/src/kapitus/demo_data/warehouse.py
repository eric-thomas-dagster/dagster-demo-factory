...
Real mode targets Snowflake (Kapitus's stated warehouse per the brief). Demo
mode targets a local DuckDB file at this path so dbt (real, not mocked) and
the ingestion components read and write the same warehouse.
...
`KAPITUS_DEMO_DUCKDB_PATH` can override it, but never gates startup.
"""
...
def demo_duckdb_path() -> str:
    path = os.environ.get("KAPITUS_DEMO_DUCKDB_PATH", DEFAULT_DUCKDB_PATH)
...
def upsert_partition(
    ...
    """Replaces just the rows matching `match` (e.g. one product line's one day).

    A partitioned ingestion asset re-running one partition must not clobber
    every other partition already in the table -- that's the entire recovery
    story (rematerialize one product line's one day, not the other four).
