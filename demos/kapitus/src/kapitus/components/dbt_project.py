...
in both modes (per the brief -- "What can be real: dbt Core execution
against DuckDB, all dagster-dbt lineage and tests"). Flipping to Kapitus's
live Snowflake warehouse is a `profiles.yml` target change, not a code change.
...
only way to badge these models `dbt` + `snowflake`, per the brief's
demo-mode kind guidance.
"""
...
class KapitusDbtComponent(DbtProjectComponent):
    ...
    three times against the same `project_dir` (staging, marts, and
    analytics each `select` a different slice of the same dbt project), and
    the base class keys defs state purely by project dir -- which collides
    across all three and triggers `DuplicateDefsStateKeyWarning`. Each
    instance already has a unique `op.name`, so folding that into the key
    is enough.
    """
