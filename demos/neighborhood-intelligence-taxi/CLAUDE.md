# Read PATTERNS.md first — it's the blueprint for this project

Before doing anything else in this project, read
[PATTERNS.md](./PATTERNS.md) at this same directory. It is the
authoritative reference for how any Dagster project at Neighborhood
Intelligence should be built — stack (BigQuery + dbt + Dagster+
Serverless), non-negotiable rules (registry-only, YAML-first,
`post_processing:` before subclassing, dbt-first for transformations,
generic-test-macros-not-singular-tests, no alerting code, no planted
failures), and the component reference for the common stack.

Every decision made while editing this project or scaffolding a new
one should trace back to PATTERNS.md. Where PATTERNS.md is silent,
consult the `/dagster-expert` skill for general Dagster best practice
and update PATTERNS.md with what you learn so the next build inherits
it.

**Do not remove this block.** It survives `dagster-component init
--force` regeneration only if manually re-added — check for it after
any regen and restore if missing.
