...
carry the story, not real data, so every asset body is a no-op. Registry
search turned up nothing for "declare a list of no-op assets from YAML" --
that's a generic authoring need, not an integration domain, so rungs 1-3 of
the component escalation ladder don't apply. See
`component-feedback/2026-08-28-graph-first-assets.md` for the searches run.
...
class GraphFirstAssetsComponent(dg.Component, dg.Resolvable, dg.Model):
    ...
    def recompute_after_upstream_checks_pass() -> dg.AutomationCondition:
        ...
        composition lives here instead. Used on the regulator-facing extract
        so it never rebuilds off data its own upstream check has failed --
        "downstream refuses to compute on bad input" (CLAUDE.md feature floor).
        """
