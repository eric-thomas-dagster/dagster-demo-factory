...
`cron_schedule` component: per LEARNINGS.md, that component's
partitioned-job mode rejects `cron_expression`/`execution_timezone`
combined with `partition_type`/`hour_of_day` -- a specific local hour can't
be expressed alongside a `partitions_def`. The native call is one function
