...
`raw_*` Dagster assets built by `RvuFivetranComponent`
(`defs/ingestion/defs.yaml`) represent those same Fivetran connector syncs
for real -- each one's demo-mode execution body reads the rows this module
already landed, the same way a real Fivetran sync would read rows already
sitting in the source system and land them in the warehouse. This module is
what makes those tables queryable so the real dbt project downstream (and
the demo-mode Fivetran sync bodies) have something to read.
