...
Real mode targets Databricks Unity Catalog. Demo mode targets a local
DuckDB file so dbt (real, not mocked) and the fixture bootstrap read and
write the same warehouse.
...
`EON_DEMO_DUCKDB_PATH` can override it, but never gates startup.
"""
...
def demo_duckdb_path() -> str:
    path = os.environ.get("EON_DEMO_DUCKDB_PATH", DEFAULT_DUCKDB_PATH)
