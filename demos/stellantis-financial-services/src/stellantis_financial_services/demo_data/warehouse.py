...
Real mode targets whatever Lakehouse/Warehouse tables the migrated Fabric
pipelines actually write to (Fabric Lakehouse Delta tables via
`fabric_lakehouse_io_manager`, or the Fabric Data Warehouse SQL endpoint).
Demo mode targets a local DuckDB file at this path so every trigger-and-observe
asset has somewhere to land the synthetic rows it stands in for.
...
set by hand. `STELLANTIS_DEMO_DUCKDB_PATH` can override it, but never gates
startup.
"""
...
def upsert_partition(
    ...
    df: pd.DataFrame,
    match: dict,
    ddl_columns: dict | None = None,
) -> None:
    """Replaces just the rows matching `match` (e.g. one day's partition).

    A partitioned asset re-running one partition must not clobber every
    other partition already in the table. `ddl_columns` (`{column:
    sql_type}`) pins the schema used the first time the table is created, so
    an empty first batch doesn't leave the table with the wrong inferred
    types for every later insert.
    """
...
def query_df(conn, sql: str, params: list | None = None) -> pd.DataFrame:
