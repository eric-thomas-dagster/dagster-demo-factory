...
Real mode targets Microsoft Fabric / OneLake (the brief's stated target
platform). Demo mode targets a local DuckDB file at this path so dbt (real,
not mocked) and the ingestion components read and write the same warehouse.
...
def upsert_partition(
    ...
    """Replaces just the rows matching `match` (e.g. one vendor's one day).
    ...
    story (rematerialize one dealer's one day, not the other 699 packages).
