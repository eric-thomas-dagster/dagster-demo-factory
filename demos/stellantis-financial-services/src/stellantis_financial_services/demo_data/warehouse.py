...
Real mode targets the Fabric-managed lakehouse the SSIS packages are being
migrated into (per the brief). Demo mode targets a local DuckDB file at this
path, standing in for the lakehouse tables every Fabric pipeline in this
demo reads and writes.

The path defaults to somewhere inside the project
(`demo_data/stellantis.duckdb`), created on first use, so the project runs
with zero setup after `git clone` + `uv sync` + `dg dev` -- no env var has to
be set by hand. `SFS_DEMO_DUCKDB_PATH` can override it, but never gates
startup.
"""
...
DEFAULT_DUCKDB_PATH = str(Path(__file__).parent / "stellantis.duckdb")
...
def upsert_partition(
    ...
    A partitioned pipeline re-running one partition must not clobber every
    other partition already in the table -- that's the entire targeted
    replay/backfill story this demo exists to show.
