...
stands in for that destination. The destination's `service` is set to
`snowflake` so the default translator's kind badge (`{"fivetran", service}`)
matches the prospect's real warehouse rather than the demo engine.
"""
