...
4. If asked "is this actually writing to Postgres?": *"Right now this
   writes to a local file standing in for Postgres because I don't have
   your database credentials — but it's the real, open-source Dagster
   Postgres IO manager underneath, the same one you'd point at your live
   database. One config flag and real credentials, and it's writing to
   your actual Postgres — nothing about the pipeline changes."*
...
1. Open its metadata panel side by side (mentally) with the Postgres
   ...
   graph. And like the Postgres side, this is the real Snowflake IO
   manager, just pointed at a stand-in warehouse until I have your
   Snowflake account."*
...
automation, the two-company partitioning, and the real Postgres and
Snowflake IO managers under the hood — works identically the day you point
those resources at your real databases and the Qlik export at your real
tenant. That's the whole demo-mode idea, and it's the same pattern behind
the migration workflow and ML pipeline you've already built yourselves."*
...
- No real Postgres, Snowflake, Qlik Cloud, or AWS credentials — everything
  demo-mode mocked, though the Postgres and Snowflake IO managers are the
  real, unmodified community-registry components, not stand-ins.
