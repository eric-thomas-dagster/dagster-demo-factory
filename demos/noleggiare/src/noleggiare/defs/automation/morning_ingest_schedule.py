...
Snowflake-variant twin, the ML forecast, the Qlik Cloud export) is
deliberately NOT on this schedule. It's `date x company` multi-partitioned,
which `build_schedule_from_partitioned_job` doesn't target directly, and
more importantly it doesn't need to be: `AutomationCondition.eager()` on the
